...
function run_ior() {
    ...
    runjob -n $NPROCS -p 16 --block $COBALT_PARTNAME --timeout 600 --envs BGLOCKLESSMPIO_F_TYPE=0x47504653 --envs DARSHAN_TOKIO_LOG_PATH=${TOKIO_JOB_DIR}/runs/darshan-logs --verbose=INFO : \
        $TOKIO_BIN_DIR/ior \
            -H \
            $IOR_CLI_ARGS \
            -s $SEGMENT_CT \
